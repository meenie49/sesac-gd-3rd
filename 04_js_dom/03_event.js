...
btn1.addEventListener('mouseover', function () {
  ...
  // 이벤트 외 행동을 해도 원래 이벤트가 유지됨 -> 여러개 이벤트 사용
...
// 같은 기능을 재사용 하는 경우에 이렇게 함수를 선언하고, addEventListenr에서 불러와 사용
// addEventListener에 함수를 불러올 때 괄호 작성 X
// 괄호 작성 시 js를 파싱하면서 이벤트가 일어나지 않아도 해당 코드와 만나면 바로 해당 함수를 실행시켜버림
function changeColor() {
  let divs = document.querySelectorAll('.container div');

  // 만약 div 요소가 없다면 alert 창 띄우는 예외처리
  if (divs.length !== 0) {
    for (let div of divs) {
      div.style.color = 'red';
    }
...
window.addEventListener('scroll', (event) => {
  ...
  console.log(scrollY); // 스크롤 위치에 따라 이벤트를 걸어줄 수 있음
...
// --------------------------------------------------------
// 변경 이벤트
const changeInput = document.querySelector('#change-event');
changeInput.addEventListener('input', (e) => {
  console.log('changed!');

  const div = document.querySelector('.change-event-value');
  // e: 이벤트 객체
