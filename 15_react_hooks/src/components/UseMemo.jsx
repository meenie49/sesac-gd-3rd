...
export default function UseMemo() {
  ...
  */
  ...
  // 그러므로 useMemo 사용해서 count의 값에 변화가 있을 때만 콜백 함수 실행시키기
  // 렌더링 과정에서 두 번째 인자로 받은 의존성 배열 내 요소의 값이 바뀌는 경우
  // 첫 번째 인자로 받은 콜백 함수가 실행 -> 리턴값을 변환
  // 즉, useMemo 훅에서 inputVal state가 변경되면 컴포넌트는 리렌더링 되지만,
  // calc 함수는 다시 실행되지 않음
  // 원리 : 해당 함수의 '반환값'을 '기억'하고 있다가 의존성 배열의 값 (count)변경 시 재실행
  // useEffect와 똑같이 count값이 바뀔 때만 사용하고 싶으면 의존성배열에 넣어주면 됨
  const calc = useMemo(() => {
    console.log('...calc');
    return count * 2;
  }, [count]);
