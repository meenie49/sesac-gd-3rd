...
// input 태그를 선택해 해당 input의 value를 가져와
function useVal() {
  // val()인자를 안넣으면 input값을 그대로 가져옴
  ...
  // js 사용
  // getElementsbyTagname 으로 했을 때는 안됨
...
function useAppend() {
  ...
  // prepend() : 선택된 요소이 첫 번째 자식으로 인자에 작성된 요소 추가
  // container의 자식 중 class가 box이고 yellow인 앞에 추가
  // $('.container').prepend(`<div class='box yellow'</div>`);

  // before() : 선택된 요소의 앞에 인자로 전달된 요소 추가
  // red 라는 클래스를 가진 모든 요소의 앞에 div.orane 요소 추가됨
...
function useParent() {
  ...
  // .container 요소이 모든 자식에게 배경색 노란색 지정
